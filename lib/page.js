...
class Page {
    ...
    async _loadPage(client) {
        ...
        await Page.enable();
        // using default window.open for auto downloading breaks page load
        // replace it with noop function
        await Page.addScriptToEvaluateOnNewDocument({source: 'window.open = function () {}'});
    ...
    _processEvents(client, fulfill, reject) {
        ...
        Network.loadingFailed(({requestId, errorText, canceled, timestamp}) => {
