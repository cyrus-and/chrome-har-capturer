...
class Page {
    ...
    async load() {
        ...
        try {
            ...
            // dispose the context
            context.destroy().catch(() => {
                // XXX not sure why but when a disconnection event happens,
                // awaiting this promise results in interrupting the whole
                // process, so it is caught (to avoid unhandled promises
                // rejection) and nothing is done
            });
        }
    }
