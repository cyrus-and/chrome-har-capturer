...
async function milliseconds(n) {
    return new Promise((fulfill, reject) => {
        ...
        setTimeout(fulfill, n, true /* timed out */);
    });
}
...
class Page {
    ...
    async load() {
        ...
        // no matter what the context must be disposed
        try {
            const {preHook, postHook} = this._options;
            const client = await context.create();
            // optionally run the user-defined hook
            if (typeof preHook === 'function') {
                await preHook(this._url, client, this._index, this._urls);
            }
            // wait for proper termination/error or timeout
            const timedOut = await Promise.race([
                this._loadPage(client),
                milliseconds(this._options.timeout)
            ]);
            // a timeout aborts this page load
            if (timedOut) {
                throw new Error('Timed out');
            }
            // optionally run the user-defined hook
            if (typeof postHook === 'function') {
                await postHook(this._url, client, this._index, this._urls);
            }
        }
        finally {
            // dispose the context
    ...
    async _loadPage(client) {
        ...
        await Promise.all([navigation, termination]);
    }
    ...
    _processEvents(client, fulfill, reject) {
        ...
        Network.requestWillBeSent((params) => {
            ...
            if (redirectResponse) {
                ...
                info.entries.get(requestId).responseParams = {
                ...
                info.entries.get(requestId).responseFinishedS = timestamp;
                ...
                info.entries.set(newId, info.entries.get(requestId));
