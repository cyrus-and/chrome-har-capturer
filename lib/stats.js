...
class Stats {
    ...
    _Network_requestWillBeSentExtraInfo(fulfill, reject, params) {
        const {requestId, headers: newHeaders} = params;
        const entry = this.entries.get(requestId);
        if (!entry) {
            return;
        }
        Object.keys(newHeaders).forEach(newHeader => {
            const existingHeader = Object.keys(entry.requestParams.request.headers).filter(existingHeader => existingHeader.toLowerCase() === newHeader.toLowerCase());
            if (existingHeader.length > 0) {
                entry.requestParams.request.headers[existingHeader] = newHeaders[newHeader];
            } else {
                entry.requestParams.request.headers[newHeader] = newHeaders[newHeader];
            }
        });
    }
