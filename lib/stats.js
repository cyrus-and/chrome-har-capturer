...
class Stats {
    ...
    _Network_responseReceived(fulfill, reject, params) {
        ...
        // We are setting the responseFinishedS with the request timestamp if the response finished successsfully with a 200 and the connection has been closed
        // forcing the requests to be included in the HAR.
        if (params.response.status === 200 && params.response.headers.Connection === 'close') {
