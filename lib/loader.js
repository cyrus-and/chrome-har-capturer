...
class Loader extends EventEmitter {
    ...
    async _handleUrl(url, index, triesLeft = this._options.retry) {
        const page = new Page({
            url, index,
            urls: this._urls,
        ...
        try {
            triesLeft -= 1;
            process.stderr.write(`Try ${this._options.retry - triesLeft} of ${this._options.retry} `);
            ...
            if (triesLeft > 0) {
                process.stderr.write(`Will try again in ${this._options.retryTimeout} ms\n`);
                return new Promise((fullfill) => {
                    setTimeout(fullfill, this._options.retryTimeout);
                }).then(() => this._handleUrl(url, index, triesLeft));
            }
            return null;
        }
        return page;
