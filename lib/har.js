...
function fromPage(page) {
    ...
    for (var requestId in page.objects) {
        ...
        // estimate the header size according to the protocol
        ...
        var location = Object.keys(object.responseMessage.response.headers).find(function (name) {
            return name.match(/location/i);
        });
        var redirectUrl = object.responseMessage.response.headers[location];
        // query string
        ...
        //    -1, e.g., HTTP/2)
        //  - object.responseLength: actual *decoded* body size
        //  - object.encodedResponseLength: headers + *encoded* body size
        //    (NOTE: this is 0 when there is no content!)
        //
        // To HAR:
        //  - headersSize: size of the headers if available (otherwise -1, e.g., HTTP/2)
        ...
        var bodySize;
        var transferSize = object.encodedResponseLength;
        if (responseHeaders.size === -1) {
            ...
            // size (see #27)
            bodySize = -1;
        } else if (object.encodedResponseLength === 0) {
            // when there is no content (e.g., 302) `encodedResponseLength` is
            // zero, that is does not take into account header information
            bodySize = 0;
            // and the on-the-wire data is thus merely the headers size
            transferSize = responseHeaders.size;
        } else {
            // otherwise the encoded body size can be obtained as follows
            bodySize = object.encodedResponseLength - responseHeaders.size;
        }
...
function convertHeaders(headers) {
    var headersObject = {'pairs': [], 'size': undefined};
