...
function create(pages) {
    ...
    for (let [pageIndex, page] of pages.entries()) {
        ...
        const log = parsePage(pageId.toString(), page.info);
...
function parseEntry(pageref, entry) {
    ...
    const serverIPAddress = response.remoteIPAddress;
    const connection = response.connectionId.toString();
...
function computeTimings(entry) {
    ...
    const time = toMilliseconds(entry.responseFinishedS - timing.requestTime);
...
function computePayload(entry, headers) {
    ...
    let bodySize;
    let compression;
    const transferSize = entry.encodedResponseLength;
    ...
    } else if (entry.encodedResponseLength === 0) {
    ...
    return {
        request: {
            ...
            bodySize: getHeaderValue(headers.request.map, 'content-length', -1)
        },
        response: {
            bodySize,
            transferSize,
...
function getRawRequest(request, headerPairs) {
    ...
    for (let {name, value} of headerPairs) {
